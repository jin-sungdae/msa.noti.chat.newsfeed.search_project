...
public class NotificationController {
    ...
    @PostMapping("/send")
    public APIDataResponse<String> sendNotification(@RequestBody RequestNotification request) {

        Optional<UserSettings> settings = redisUserSettingsRepository.getUserSettings(request.getUserId());

        if (notificationService.shouldSendNotification(settings.orElse(new UserSettings("Y", "Y", "Y", "kr")), request.getNotificationType())) {
    ...
    public APIDataResponse<Map<String, String>> sendBatchNotification(@RequestBody List<RequestNotification> requests) {

        List<String> userIdList = requests.stream().map(RequestNotification::getUserId).collect(Collectors.toList());

        Map<String, UserSettings> userSettingsMap = redisUserSettingsRepository.getUsersSettings(userIdList);
        Map<String, String> response = new HashMap<>();

        for (RequestNotification request : requests) {
