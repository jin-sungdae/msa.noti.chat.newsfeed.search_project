                                        ...
                                        <div class="col-lg-12 row">
                                            ...
                                            <div class="col-lg-7 pt-3 row" id="showIsIntroN">
                                                <div class="col-lg-2 row">
                                                    <div class="col">
                                                        <div class="form-check form-check-custom form-check-warning form-check-solid">
                                                            <input class="form-check-input" type="radio" name="isIntro" value="N" ${list.isIntro eq 'N' || empty list ? 'checked' : ''} id="isIntroN" ${list.isOpen eq 'N' || empty list ? 'disabled' : ''}/>
                                                    ...
                                                    <div class="col">
                                                        <div class="form-check form-check-custom form-check-warning form-check-solid">
                                                            <input class="form-check-input" type="radio" name="isIntro" value="Y"  ${list.isIntro eq 'Y' ? 'checked' : ''} id="isIntroY" ${list.isOpen eq 'N' || empty list ? 'disabled' : ''}/>
                                                ...
                                                <div class="col-lg-2  d-flex align-items-center justify-content-center">
                                                    <div class="col-lg-12">
                                                          <input
                                                                ...
                                                                id="detailTitle1"
                                                                name="detailTitle1"
                                                                class="form-control ${list.referenceBookDetail[1].isUse eq 'Y' ? '' : 'form-control-solid'} textModifyDetail"
                                                                placeholder="탭 명칭 입력"
                                                                maxlength="12"
                                                ...
                                                <div class="col-lg-2  d-flex align-items-center">
                                                    <div class="col-lg-12">
                                                        <input
                                                                ...
                                                                id="detailTitle3"
                                                                name="detailTitle3"
                                                                class="form-control ${list.referenceBookDetail[3].isUse eq 'Y' ? '' : 'form-control-solid'} textModifyDetail"
                                                                placeholder="탭 명칭 입력"
                                                                minlength="2"
                                                                maxlength="12"
...
<script type="text/javascript">
        ...
        ntdrop[1] = global.genDropzone("#dz1",
            {
                ...
                messages : {
                    empty : 'PDF지문을 등록해주세요.',
                    accept: '* 파일용량 및 PDF 파일 유형을 다시 확인해주세요.',
                    count: '',
                    size: '* 파일용량 및 PDF 파일 유형을 다시 확인해주세요.'
                }
            });
    ...
    function validateIsIntro(introNum) {
        ...
        console.log('value ====', value, 'intro nUm === ', introNum)
        if (value === 'Y' && introNum >= 10) {
    ...
    async function save(element) {
        ...
        if (!isValid || status === 'Invalid' || (introNum >= 10 && value === 'Y')) {
            console.log('intro Num =====', introNum)
        ...
        const bookCode = getValue('bookCode');
        const bookCodeName = getValue('bookCodeName');

        const grade = getValue('grade');
        const semester = getValue('semester');
        const subject = getValue('subject');
        const seriesCode = getValue('seriesCode');
        const seriesCodeName = getValue('seriesCodeName');
        const year = getValue('year');
        const revisionCurriculum = getValue('revisionCurriculum');
        const referenceBookTitle = getValue('referenceBookTitle');
        const bookCharacter = getValue('bookCharacter');
        const author = getValue('author');
        const bookStructure = getValue('bookStructure');
        const width = getValue('width');
        const height = getValue('height');
        const chromacity = getValue('chromacity');
        const price = getValue('price');
        const preview = document.querySelector('input[name="preview"]:checked').value;
        const previewUrl = preview === 'URL' ? document.getElementById('previewUrl').value : '';
        const gangnamBroadcastUrl = getValue('gangnamBroadcastUrl');
        const referenceBookDetail = getValue('referenceBookDetail');
        const detailTitle1 = getValue('detailTitle1');
        const detailContent1 = getValue('detailContent1');
        const detailTitle2 = getValue('detailTitle2');
        const detailContent2 = getValue('detailContent2');
        const detailTitle3 = getValue('detailTitle3');
        const detailContent3 = getValue('detailContent3');

        const description = getValue('seriesDescription');

        console.log('hihi')
        const isRequestSample = document.querySelector('input[name="isRequestSample"]:checked').value;
        ...
        const isUse1 = document.querySelector('input[name="isUse1"]:checked').value;
        const isUse2 = document.querySelector('input[name="isUse2"]:checked').value;
        const isUse3 = document.querySelector('input[name="isUse3"]:checked').value;

        const etcData2 = document.getElementById('etcData2').value;
        const etcData3 = document.getElementById('etcData3').value;
        let data = {
            reference : {
                uid : uid,
                bookCode : bookCode,
                bookCodeName : bookCodeName,
                school : school,
                grade : grade,
                semester : semester,
                subject : subject,
                seriesCode : seriesCode,
                seriesCodeName : seriesCodeName,
                year : year,
                ...
                isOpen : isOpen,
                isIntro : isIntro,
                gangnamBroadcastUrl : gangnamBroadcastUrl,
            ...
            referenceDetailList : [
                {
                    detailTitle : '지도서 소개',
                    isUse: 'Y',
                    detailContent : referenceBookDetail,
                    sortOrder : '1'

                },
                {
                    detailTitle : detailTitle1,
                    isUse: isUse1,
    ...
    async function fileFormData(uid) {
        const dropzones = [
            ntdrop[0].files, ntdrop[1].files, ntdrop[2].files, ntdrop[3].files,
            ntdrop[4].files, ntdrop[5].files, ntdrop[6].files, ntdrop[7].files,
            ntdrop[8].files, ntdrop[9].files, ntdrop[10].files, ntdrop[11].files,
            ntdrop[12].files, ntdrop[13].files, ntdrop[14].files
        ];
        ...
        const fileSettings = {
            ...
            'STUDENT-CORRIGENDUM': {
                DZID : '2',
                maxSizeMB: 300,
                allowedExtensions: /\.(mp3|mp4|pdf|jpg|jpeg|png|hwp|hwpx|ppt|pptx|doc|docx|zip)$/i,
                type: 'STUDENT-CORRIGENDUM',
                description: '학습자료 - (학생용) 정오표',
                extension: '.mp3 .mp4 .pdf .jpg .jpeg .png .hwp .hwpx .ppt .pptx .doc .docx .zip .egg',
                sortOrder : '3'
            },
            'STUDENT-ANSWER': {
                DZID : '3',
        ...
        for (let i = 0; i < dropzones.length; i++) {
            ...
            console.log(dropzoneElement.style.display);
    ...
    async function inputDeleteData(settings, fileData, index) {
        ...
        fileData.append('ParentTable', 'REFERENCE_BOOK');
    ...
    //TODO : PDF 등록 되어있을때 URL 경로 변경시 파일 삭제
    async function modify(element) {
        ...
        let isValid = errorValidation();
        const value = document.querySelector('input[name="isIntro"]:checked').value;
        console.log(isValid, status, introNum, value);
        if (!isValid || status === 'Invalid' || (introNum >= 10 && value === 'Y')) {
            monitorDropzoneFiles(introNum);
            windowUpScroll();
            isSubmitting = false;
