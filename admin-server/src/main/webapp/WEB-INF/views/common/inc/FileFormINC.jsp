...
<%@ page import="com.user.server.util.NTUtil" %>
...
%>
<div class="table-responsive test" style="display: ${(empty fileList || fileList[0].uid eq '') && (parentUid ne '') && viewType eq 'view' ? 'none' : 'block'}">
    ...
    <thead>
        <tr class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
            ...
            <c:if test="${viewType ne 'view'}">
                <th class="pt-0 min-w-50px view" style="display: ${viewType ne 'view' && not empty parentUid ? 'none' : 'block'}">삭제</th>
            </c:if>
            <th class="pt-0 min-w-125px">파일명</th>
            <%if(isSecureSelect) {%><th class="pt-0 min-w-50px">보안</th><%} %>
            <%--<th class="pt-0 min-w-125px">대체텍스트</th>--%>
            <c:if test="${fileList[0].fileExtension eq ('jpg' || 'png' || 'gif' || 'bmp' || 'svg' || 'jpeg')}">
...
<c:forEach items="${fileList}" var="row" varStatus="status">
    <tr id="dropzoneImage<%=DZID %>"<%if(hasOrder){ %> class="nestable-list-item"<%} %>>
        ...
        <c:if test="${viewType ne 'view'}">
            <td width="43" class="text-center  view">
                    ${viewType}
                <input
                        type="hidden"
                        class="p_fparam"
                ...
                <a class="btn btn-sm btn-icon btn-warning" data-id="dropzone<%=DZID %>" onclick="global.dropzoneDeleteFile(this)"><i class="icon-xl fas fa-times"></i></a>
        ...
        <td>
                <c:if test="${row.fileType eq 'image' }">
                    <a href="${uploadUrl}${row.getSaveFilePath() }" target="_blank" id="valid<%=DZID %>" style="color:#F1BC00;">${row.fileName }
                        <c:if test="${row.uid ne null and row.uid ne ''}"> (${row.uid})</c:if>
                    </a>
                </c:if>

                <c:if test="${row.fileType ne 'image' }">
                    <a href="${uploadUrl}${row.getSaveFilePath() }" download="${row.fileName }" id="valid<%=DZID %>" type="_blank" style="color:#F1BC00;">${row.fileName }<c:if test="${row.uid ne null and row.uid ne ''}"> (${row.uid})</c:if></a>
<%--                <a href="/FileDownload?uid=${row.uid }" id="valid<%=DZID %>" style="color:#F1BC00;">${row.fileName }--%>
<%--                    <c:if test="${row.uid ne null and row.uid ne ''}"> (${row.uid})</c:if>--%>
<%--                </a>--%>
                </c:if>
<%--                <input type="text" class="form-control form-control-solid p_fparam" name="Description" value="${row.description }" placeholder="대체 텍스트" maxlength="500" />--%>
            ...
            <%} %>
                ...
                <c:choose>
                    <c:when test="${row.fileExtension eq 'jpg' or row.fileExtension eq 'png' or row.fileExtension eq 'gif' or row.fileExtension eq 'bmp' or row.fileExtension eq 'svg' or row.fileExtension eq 'jpeg'}">
                    <td width="140">
	...
	<tfoot>
		<tr>
			<td class="pt-0" colspan="<%=(hasOrder)?((isSecureSelect)?4:3):((isSecureSelect)?4:3)%>" id="<%=DZID %>" style="cursor:pointer;text-align:center;">
...
<c:if test="${viewType eq 'view'}">
    <div class="table-responsive pt-3 notView" style="display: ${(empty fileList || fileList[0].uid eq '') && (parentUid ne '') ? 'block' : 'none'}">
        -
    </div>
</c:if>
