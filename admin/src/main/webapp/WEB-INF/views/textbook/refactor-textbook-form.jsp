...
<script type="text/javascript">
    ...
    function collectData() {
        ...
        detailSections.forEach((section, index) => {
            ...
            let isUse = section.querySelector('input[type="radio"]:checked')?.value || 'N';
    ...
    function setRequiredForNtdrop1() {
        ntdrop[1].destroy();
        const newOptions = {...ntdrop[1].options, required: 'Y'};

        console.log('newOptions ====', newOptions);
        ntdrop[1] = global.genDropzone("#dz1", newOptions);
    }
    ...
    async function validateSchoolCode(school) {
        try {
    ...
    function validateFiles(previewValue) {
        if (previewValue === 'PDF') {
            return dropzoneCheck(previewValue);
        }
        return dropzoneCheck();
    }
    ...
    async function validateForm() {
        ...
        return isValid && status === 'Valid';
    }

    async function submitData(previewValue, data) {
        if (previewValue === 'URL') {
            const el = document.getElementById('dropzoneImagedz1');
            el?.querySelector('td > a')?.click();
        }
    ...
    async function save(element) {
        ...
        const introNum = await validateSchoolCode(school);
        const previewValue = document.querySelector('input[name="preview"]:checked').value;
        const isFileValid = validateFiles(previewValue);
        const isFormValid = await validateForm();

        if (!isFormValid || !isFileValid || status === 'Invalid' ||  (introNum >= 10 && document.querySelector('input[name="isIntro"]:checked').value === 'Y')) {
            focusAndScroll();
            isSubmitting = false;
            return;
        }
        ...
        try {
            ...
            const result = await submitData(previewValue, data);
